...
<!-- Вставте замість старої секції AR у ui.js (починаючи від // =========================================================) -->
// =========================================================
// ДОПОВНЕНА РЕАЛЬНІСТЬ (AR) — ВИПРАВЛЕНО
// =========================================================
let xrSession = null;
let xrRenderer, xrCamera, xrScene;
let graphGroup;
let meshes = {};
let isARAddMode = false;
let selectedForARConnection = null;

function initARMode() {
    if (!('xr' in navigator)) {
        showToast("WebXR не підтримується вашим браузером");
        startVideoARFallback();
        return;
    }
    ...
    navigator.xr.isSessionSupported('immersive-ar').then(supported => {
        if (supported) {
            ...
            showToast("Immersive AR не підтримується. Запускаю 3D-фолбек.");
            startVideoARFallback();
        }
    }).catch(() => startVideoARFallback());
}

async function startTrueWebXR() {
    try {
        ...
        graphGroup.position.set(0, 0, -1.2);   // 1.2м перед користувачем
        graphGroup.scale.set(0.002, 0.002, 0.002);
        xrScene.add(graphGroup);

        // Створюємо DOM overlay
        const overlay = createAROverlay();
        ...
        window.refreshAR = refreshARGraph;
        refreshARGraph();
        ...
        xrRenderer.setAnimationLoop((timestamp, frame) => {
            if (frame) {
                graphGroup.children.forEach(child => {
                    if (child.userData?.isNode) child.rotation.y += 0.015;
                });
            }
        ...
        const rayLine = new THREE.Line(
            ...
            new THREE.LineBasicMaterial({ color: 0xff7eb3 })
        );
        rayLine.scale.z = 3;
        ...
        showToast("Помилка WebXR. Перехід на 3D симуляцію.");
        cleanupAR();
        startVideoARFallback();
    }
}
...
function createAROverlay() {
    ...
    div.style.cssText = `
        ...
        z-index: 1000; pointer-events: none; color: white; font-family: Inter, sans-serif;
    `;

    div.innerHTML = `
        <div style="position:absolute; bottom:30px; left:30px; background:rgba(0,0,0,0.75); padding:15px 20px; border-radius:12px; pointer-events:auto;">
            <h4 style="color:var(--accent-pink); margin:0 0 8px 0;">WebXR AR Режим</h4>
            <button onclick="toggleARAddMode()" class="btn outline-pink" id="ar-add-btn" style="margin-right:8px;">
                Режим зв'язків: ВИМК
            </button>
            <button onclick="endXRSession()" class="btn danger">Вийти</button>
        </div>
    `;
    return div;
}

function toggleARAddMode() {
    isARAddMode = !isARAddMode;
    const btn = document.getElementById('ar-add-btn');
    if (btn) {
        btn.textContent = isARAddMode ? "Режим зв'язків: УВІМК" : "Режим зв'язків: ВИМК";
        btn.style.background = isARAddMode ? "var(--accent-pink)" : "transparent";
    }
    selectedForARConnection = null;
    showToast(isARAddMode ? "Оберіть дві вершини" : "Режим перегляду");
}
...
function onARSelect(event) {
    ...
    const sphereIntersect = intersects.find(i => i.object.userData?.isNode);

    if (!sphereIntersect) return;

    const nodeId = sphereIntersect.object.userData.id;
    const person = people.find(p => p.id === nodeId);

    if (isARAddMode) {
        handleARConnection(nodeId, person);
    } else {
        // Просто показати інформацію
        showToast(`${person.name} — ${person.interests.join(', ')}`);
        sphereIntersect.object.material.emissive?.setHex(0x8e2de2);
        setTimeout(() => {
            if (sphereIntersect.object.material) sphereIntersect.object.material.emissive.setHex(0x2a0815);
        }, 2000);
    }
}

function handleARConnection(nodeId, person) {
    if (!selectedForARConnection) {
        selectedForARConnection = { id: nodeId, name: person.name };
        showToast(`Вибрано: ${person.name}. Оберіть другу вершину.`);
    } else {
        if (selectedForARConnection.id !== nodeId) {
            const added = addEdge(selectedForARConnection.id, nodeId);
            showToast(added ? "Зв'язок створено!" : "Зв'язок видалено");
            if (!added) removeEdge(selectedForARConnection.id, nodeId);
            refreshARGraph();
            updateGraphElements();
        }
        selectedForARConnection = null;
...
function refreshARGraph() {
    ...
    people.forEach(p => {
        ...
        sphere.userData = { id: p.id, name: p.name, isNode: true };
...
// ==================== ФОЛБЕК (старий 3D) ====================
function startVideoARFallback() {
    // ... ваш старий код startVideoAR() залишається без змін ...
    // (або можете залишити як є)
    showToast("Запущено 3D-симуляцію з OrbitControls");
    // Тут можна вставити ваш старий startVideoAR() якщо потрібно
}
